      ...
      <section className="py-16 bg-[var(--gray-100)]">
        <div className="max-w-3xl mx-auto px-6">
          <div className="bg-white rounded-2xl border border-[var(--gray-200)] shadow-lg overflow-hidden">
            {/* Report header */}
            <div className="p-8 border-b border-[var(--gray-200)] text-center">
              <div className="w-16 h-16 rounded-full border-2 border-[var(--cyan)] flex items-center justify-center mx-auto mb-4">
                <span className="text-[var(--cyan)] font-bold text-xs font-[Outfit]">TYW</span>
              </div>
              <h2 className="text-2xl font-[Outfit] font-bold tracking-wide">AIR FILTER REPORT</h2>
            </div>

            {/* Client info */}
            <div className="p-8 border-b border-[var(--gray-200)]">
              <div className="space-y-1.5 text-sm">
                <p><strong>Name:</strong> Test Thetest</p>
                <p><strong>Address:</strong> <span className="bg-[var(--gray-200)] px-2 py-0.5 rounded text-xs">███████████████</span></p>
                <p><strong>Email:</strong> <span className="bg-[var(--gray-200)] px-2 py-0.5 rounded text-xs">███████████████</span></p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--gray-200)]">
                      <th className="text-left py-3 pr-4 font-[Outfit] font-semibold text-[var(--gray-500)]">Category</th>
                      <th className="text-left py-3 pr-4 font-[Outfit] font-semibold text-[var(--gray-500)]">Status</th>
                      <th className="text-left py-3 font-[Outfit] font-semibold text-[var(--gray-500)]">What This Means</th>
                    </tr>
                  </thead>
                  <tbody>
                    {findings.map((f) => (
                      <tr key={f.category} className="border-b border-[var(--gray-100)]">
                        ...
                        <td className="py-3 text-[var(--gray-500)]">{f.meaning}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div className="bg-[var(--gray-100)] rounded-xl p-5">
                  <p className="text-xs text-[var(--gray-400)] mb-2 font-semibold uppercase">☁️ Current Outdoor Air Quality</p>
                  <p className="text-sm"><strong>AQI:</strong> <span className="text-green-600 font-semibold">83 (Excellent air quality)</span></p>
                  <p className="text-sm text-[var(--gray-500)]">PM2.5: 4.54 micrograms/m³</p>
                </div>
                <div className="bg-[var(--gray-100)] rounded-xl p-5">
                  <p className="text-xs text-[var(--gray-400)] mb-2 font-semibold uppercase">🌸 Current Pollen Levels</p>
                  <p className="text-sm text-[var(--gray-500)]">Seasonal data — varies by region and time of year.</p>
                </div>
              </div>
              <div className="bg-blue-50 rounded-xl p-4 text-sm text-[var(--gray-600)]">
                <strong>Your Home:</strong> 3,000–4,000 sq ft single family house · 4 occupants · HVAC
              </div>
            </div>
          </div>

          <p className="text-center text-sm text-[var(--gray-400)] mt-8">
